...
def visualize_data(X):
    ...
    for i in range(0, len(X.columns)):
        ...
        image_path = os.path.join(IMAGE_DIR,X.columns[i]+".png")
        plt.savefig(image_path,bbox_inches='tight',pad_inches = 0,dpi=200)
        plt.clf()

    return

def visualize_model(label, X):
    IMAGE_DIR = os.path.join(".","images")
    ...
    for i in range(0, len(X)):
        ...
        i*2/len(X), 0.5, X[i],  # x, y, text,
        ha='center', va='bottom'   # text alignment
        )

    image_path = os.path.join(IMAGE_DIR,"model.png")
    plt.savefig(image_path,bbox_inches='tight',pad_inches = 0,dpi=200)
    plt.clf()
...
def model_train(test=False):
    ...
    print(eval_test["0.0"]["f1-score"])
    label = []
    f1_score = []
    label.append("clf")
    f1_score.append(eval_test["0.0"]["f1-score"])

    print("... auto gamma svc")
    svc = SVC(gamma='auto')
    pipe2 = Pipeline(steps=[('pre', preprocessor),
                           ('svc',svc)])
    pipe2.fit(X_train, y_train)
    y_pred2 = pipe2.predict(X_test)
    pipe2.fit(X_train, y_train)
    eval_test2 = classification_report(y_test,y_pred2,output_dict=True)
    print(eval_test2["0.0"]["f1-score"])
    label.append("svc")
    f1_score.append(eval_test2["0.0"]["f1-score"])

    visualize_model(label, f1_score)

    
    ## retrain using all data
    if (eval_test["0.0"]["f1-score"] < eval_test2["0.0"]["f1-score"]):
        pipe = pipe2
